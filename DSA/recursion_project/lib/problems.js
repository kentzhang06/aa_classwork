...
// base case length=0 -> 0
// base case length=1 -> arr[0]
// iterative step
...
// iterative step: reverseString(str.slice(1)) + str[0];
...
function pow(base, exponent) {
  ...
  if (exponent > 0) {
    ...
    return (1.0 / base) * (1.0 / pow(base, -1 * exponent - 1));
  }
}
...
function fileFinder(directories, targetFile) {
  for (let directory in directories) {
    if (
      ...
      fileFinder(directories[directory], targetFile)
    ) {
      return true;
    }
  }
...
function pathFinder(directories, targetFile) {
  for (let directory in directories) {
    if (directory === targetFile) return '/' + targetFile;

    let subdir = directories[directory];
    let res = pathFinder(subdir, targetFile);
    // if target is found in subdir
    if (res !== null) {
